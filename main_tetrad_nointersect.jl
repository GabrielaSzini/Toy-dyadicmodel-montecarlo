...
@everywhere function simulation(grid_points, N, θ₀, αₗ, αₕ, λ₀, λ₁, x₁_value, x₂_value, subnet_size, bmax, bmin)
    ...
    # Subset subnetworks: FUNCTION IS DIFFERENT IF WE DO NO CONSIDER GRAHAM'S TETRADS
    combinations = combination_condition(N, Y)
    # Take only the combinations that are disjoint: FUNCTION IS DIFFERENT IF WE DO NO CONSIDER GRAHAM'S TETRADS
    combinations_final = disjointcombinations(combinations)
    ...
    fix_x_matrix, fix_y_matrix = fix_yx_graham(combinations_final, Y, X)
    ...
    result_l_y, result_u_y = mainopt_graham(total_y_grid, combinations_final, fix_x_matrix, total_a_grid, θ₀, a_i_matrix, a_j_matrix, total_y_grid_matrix, bmax, bmin, m)
    ...
    ### Extra results
    number_combinations = length(combinations)
    number_combinations_final = length(combinations_final)
    ...
    return result_l_y_mean, result_u_y_mean, true_marginal, number_combinations, number_combinations_final, percentage_links, average_links_subnetwork
...
writedlm("results/outN$(N)al$(αₗ)ah$(αₕ)lambda0$(λ₀)lambda1$(λ₁)sims$(sims)_graham.csv", result, ',')
